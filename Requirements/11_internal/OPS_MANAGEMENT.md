...
## REQ-OPS-004: Human-in-the-Loop (HITL) Triggers

**Priority:** Must | **Actors:** ENTERPRISE_ADMIN, BRAND_ADMIN, LOCATION_MGR

#### Context & Business Intent
Certain operational anomalies or high-stakes changes require explicit human approval before the system proceeds. These HITL triggers prevent automated errors in pricing or unauthorized asset changes.

#### HITL: Pricing Variance
Triggered when a menu item price change (manual or AI-suggested) exceeds `pos.pricing_variance_threshold_pct` (default: 20%).
- **Action:** Pause propagation to all channels (POS, E-Com, Aggregators).
- **Notification:** Send "Urgent: Pricing Variance Approval Required" to ENTERPRISE_ADMIN.
- **Resolution:** Admin must [Approve] or [Reject]. If Rejected, price reverts to last known good state.

#### HITL: Asset Approval
Triggered when a merchant uploads or replaces critical branding assets (Logo, Favicon, Banner) for a Microsite.
- **Action:** Asset is stored in `PENDING` state in GCS.
- **Notification:** Send "Asset Approval Required" to BRAND_ADMIN.
- **Resolution:** Admin must [Approve] to move asset to `ACTIVE` production bucket or [Reject] to delete.
