...
class AuthService {
    ...
    async login(tenantId: string, username: string, password: string): Promise<AuthTokens> {
        ...
        const isPasswordValid = await this.userService.comparePassword(password, user.passwordHash);
        if (!isPasswordValid) {
            throw new AuthError('Invalid credentials.', 'INVALID_CREDENTIALS');
        }

        const accessToken = this.tokenService.generateAccessToken({ userId: user.id, tenantId: user.tenantId, username: user.username, permissions: user.permissions });
        const refreshToken = this.tokenService.generateRefreshToken({ userId: user.id, tenantId: user.tenantId, username: user.username });

        await this.userService.updateUser(user.id, { ...user, lastLoginAt: new Date() });
    ...
    async validateToken(token: string): Promise<User | null> {
        ...
        try {
            ...
            if (!user) {
                // User no longer exists – surface as PermissionError so authorize() can pass it through
    ...
    async refreshToken(oldRefreshToken: string): Promise<AuthTokens> {
        ...
        try {
            ...
            await this.tokenService.invalidateToken(oldRefreshToken);

            const newAccessToken = this.tokenService.generateAccessToken({ userId: user.id, tenantId: user.tenantId, username: user.username, permissions: user.permissions });
            const newRefreshToken = this.tokenService.generateRefreshToken({ userId: user.id, tenantId: user.tenantId, username: user.username });
    ...
    async register(tenantId: string, username: string, password: string, email: string): Promise<User> {
        ...
        const existingUser = await this.userService.findByUsername(tenantId, username);
        if (existingUser) {
        ...
        try {
            const createdUser = await this.userService.createUser(newUser as User);
            return createdUser;
        } catch (error) {
    ...
    async logout(token: string): Promise<void> {
        ...
        try {
            ...
            await this.cacheService.del(`user:${decoded.userId}:token:${token}`);
        } catch (error) {
...
describe('AuthService Edge Cases (SEC-009: Auth Latency SLA)', () => {
    ...
    describe('4. Concurrent request handling', () => {
        ...
        test('should handle multiple refresh token requests concurrently (one-time use semantics)', async () => {
            ...
            // All 5 concurrent calls pass through verifyRefreshToken so verifyRefreshToken ×5
            // and findById ×5 match the test assertions. The one-time-use enforcement
            // happens at invalidateToken (×5 calls, 4 throw), causing 4 rejections.
            mockTokenService.verifyRefreshToken.mockImplementation((token) => {
