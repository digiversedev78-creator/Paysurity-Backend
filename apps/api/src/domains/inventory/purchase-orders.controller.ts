...
import {
  ...
  ReceivePurchaseOrderDto, // DTO for matching invoices
...
export class PurchaseOrdersController {
  ...
  async findOne(@Param('id') id: string, @Request() req: any) {
    const tenantId = req?.user?.tenantId; // Extract tenantId
    const purchaseOrder = await this.purchaseOrdersService.findOne(tenantId, id);
    if (!purchaseOrder) {
  ...
  async update(@Param('id') id: string, @Body() updatePurchaseOrderDto: UpdatePurchaseOrderDto, @Request() req: any) {
    const tenantId = req?.user?.tenantId; // Extract tenantId
    const updatedPurchaseOrder = await this.purchaseOrdersService.update(tenantId, id, updatePurchaseOrderDto);
  ...
  async receive(@Param('id') id: string, @Body() receivePurchaseOrderDto: ReceivePurchaseOrderDto, @Request() req: any) {
    const tenantId = req?.user?.tenantId; // Extract tenantId
    const receivedPurchaseOrder = await this.purchaseOrdersService.receivePurchaseOrder(tenantId, id, receivePurchaseOrderDto);
