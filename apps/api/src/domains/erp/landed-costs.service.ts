...
export class LandedCostService {
  ...
  async allocateLandedCost(tenantId: string, dto: LandedCostDto) {
    ...
    const splitAllocationPerUnitCents = Math.round(dto.totalCostCents / (moveRes.rows[0].quantity || 1));
    ...
    try {
      ...
      await (this.db as any).execute(
        ...
        [splitAllocationPerUnitCents, moveRes.rows[0].product_id, tenantId]
      );
      ...
      this.logger.log(`Successfully completed Landed Cost AVCO pad for product ${moveRes.rows[0].product_id}.`);
