...
import { erpWarehouses, erpStockMoves, erpStockQuants, erpJournalEntries, erpJournalItems, erpAccounts } from '@paysurity/database';
...
export class ErpService {
  ...
  async getWarehouses(tenantId: string) {
    return this.db.select().from(erpWarehouses).where(eq(erpWarehouses.tenantId, tenantId));
  }

  async createWarehouse(tenantId: string, body: {
    name: string; locationType?: string; parentId?: string; valuationMethod?: string;
  }) {
    const res = await this.db.insert(erpWarehouses).values({
      tenantId,
      name: body.name,
  ...
  async getStockSnapshot(tenantId: string) {
    return this.db.select().from(erpStockQuants).where(eq(erpStockQuants.tenantId, tenantId));
  }

  async getProductStock(tenantId: string, productId: string) {
    return this.db.select().from(erpStockQuants)
  ...
  async getAccounts(tenantId: string) {
    return this.db.select().from(erpAccounts).where(eq(erpAccounts.tenantId, tenantId));
  }

  async createAccount(tenantId: string, body: { code: string; name: string; accountType: string; reconcilable?: boolean }) {
    const res = await this.db.insert(erpAccounts).values({
  ...
  private async ensureChartOfAccounts(tenantId: string) {
    const existing = await this.db.select().from(erpAccounts).where(eq(erpAccounts.tenantId, tenantId)).limit(1);
    ...
    await this.db.insert(erpAccounts).values(starter as any);
  }

  // ── Accounting / Journals ─────────────────────────────────────────────────

  async getJournals(tenantId: string) {
    return this.db.select().from(erpJournalEntries).where(eq(erpJournalEntries.tenantId, tenantId));
  }

  /**
   * REQ-ERP-001: generatePosJournalEntry — real double-entry with FK-valid account IDs.
  ...
  async generatePosJournalEntry(
    ...
    const accounts = await this.db.select().from(erpAccounts).where(eq(erpAccounts.tenantId, tenantId));
    ...
    if (!cashAccountId || !salesAccountId || !cogsAccountId || !inventoryAccountId) {
      throw new Error('Chart of Accounts incomplete — required accounts missing');
    }

    const entryResult = await this.db.insert(erpJournalEntries).values({
    ...
    await this.db.insert(erpJournalItems).values([
  ...
  /**
   * REQ-ERP-003: Real trial balance — aggregates debit/credit totals from journal items per account.
   */
  ...
  async generateInvoice(tenantId: string, orderId: string) {
    // URL stub — real PDF generation deferred to a future sprint
