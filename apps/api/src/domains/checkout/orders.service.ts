...
import { orders, orderItems } from '@paysurity/database'; // Assuming these are exported from the database package
...
export class OrdersService {
  ...
  async create(createOrderDto: CreateOrderDto) {
    ...
    const totalCents = Math.round((parseFloat(createOrderDto.totalAmount) || 0) * 100);
    const [newOrder] = await this.db
      .insert(orders)
      .values({
        tenantId: createOrderDto.tenantId,
        totalCents,
        status: (createOrderDto.status || 'PENDING').toUpperCase(),
      })
      .returning();
    ...
    if (createOrderDto.items && createOrderDto.items.length > 0) {
      ...
      createdItems = await this.db.insert(orderItems).values(itemsToInsert).returning();
    }
  ...
  async remove(id: string, tenantId: string) {
    ...
    // Assuming `orderItems` has an `onDelete: 'cascade'` foreign key constraint
