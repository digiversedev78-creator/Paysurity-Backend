...
type NewPurchaseOrder = any;
type UpdatePurchaseOrder = Partial<NewPurchaseOrder>;

type PurchaseOrderItem = any;
type NewPurchaseOrderItem = any;
...
export class PurchaseOrdersService {
  ...
  async createPurchaseOrder(tenantId: string, data: CreatePurchaseOrderDto): Promise<PurchaseOrder> {
    return (this.db as any).transaction(async (tx) => {
      ...
      const [newOrder] = await tx.insert((schema as any).purchaseOrders).values(orderToInsert).returning();
      ...
      if (items && items.length > 0) {
        ...
        await tx.insert((schema as any).purchaseOrderItems).values(orderItemsToInsert).returning();
      }
      return newOrder;
    });
  }
  ...
  async findAllPurchaseOrders(tenantId: string): Promise<PurchaseOrder[]> {
    return (this.db as any).query.purchaseOrders.findMany({
      where: eq((schema as any).purchaseOrders.tenantId, tenantId),
      with: {
  ...
  async findPurchaseOrderById(id: string, tenantId: string): Promise<PurchaseOrder> {
    const purchaseOrder = await (this.db as any).query.purchaseOrders.findFirst({
      where: and(eq((schema as any).purchaseOrders.id, id), eq((schema as any).purchaseOrders.tenantId, tenantId)),
      with: {
        items: {
          with: {
  ...
  async updatePurchaseOrder(id: string, tenantId: string, data: UpdatePurchaseOrderDto): Promise<PurchaseOrder> {
    const [updatedOrder] = await (this.db as any) // STRICT RULE: (this.db as any) for Drizzle updates
      .update((schema as any).purchaseOrders)
  ...
  async addItemsToPurchaseOrder(
    ...
    return (this.db as any).transaction(async (tx) => {
      const existingOrder = await tx.query.purchaseOrders.findFirst({
        where: and(eq((schema as any).purchaseOrders.id, purchaseOrderId), eq((schema as any).purchaseOrders.tenantId, tenantId)),
      });
      ...
      const newItems = await tx.insert((schema as any).purchaseOrderItems).values(orderItemsToInsert).returning();
      ...
      const currentItems = await tx.query.purchaseOrderItems.findMany({
        where: eq((schema as any).purchaseOrderItems.purchaseOrderId, purchaseOrderId),
      });
      const newTotalAmount = currentItems.reduce((sum, item) => sum + parseFloat(item.subtotal as any), 0).toFixed(2);

      await tx.update((schema as any).purchaseOrders)
        .set({ totalAmount: newTotalAmount, updatedAt: new Date() })
        .where(eq((schema as any).purchaseOrders.id, purchaseOrderId));
    ...
    return (this.db as any).transaction(async (tx) => {
      ...
      if (data.orderedQuantity !== undefined && data.unitPrice !== undefined) {
        ...
        const currentItem = await tx.query.purchaseOrderItems.findFirst({
          where: and(eq((schema as any).purchaseOrderItems.id, itemId), eq((schema as any).purchaseOrderItems.purchaseOrderId, purchaseOrderId)),
        });
        if (!currentItem) {
      ...
      const [updatedItem] = await tx
        .update((schema as any).purchaseOrderItems)
        .set({ ...data, subtotal })
        .where(and(eq((schema as any).purchaseOrderItems.id, itemId), eq((schema as any).purchaseOrderItems.purchaseOrderId, purchaseOrderId)))
        .returning();
    ...
    return (this.db as any).transaction(async (tx) => {
      ...
      for (const receivedItem of receivedItems) {
        ...
        await tx
          ...
          .where(eq((schema as any).purchaseOrderItems.id, orderItem.id));

        // 4. Update inventory item's current stock
        await tx
          .update((schema as any).inventoryItems)
          .set({ currentStock: sql`${(schema as any).inventoryItems.currentStock} + ${receivedItem.quantityReceived}` })
          .where(and(eq((schema as any).inventoryItems.id, orderItem.inventoryItemId), eq((schema as any).inventoryItems.tenantId, tenantId))); // Ensure tenant isolation for inventory update
  ...
  async findPurchaseOrdersByVendor(vendorId: string, tenantId: string): Promise<PurchaseOrder[]> {
    ...
    const vendorExists = await (this.db as any).query.vendors.findFirst({
      where: and(eq((schema as any).vendors.id, vendorId), eq((schema as any).vendors.tenantId, tenantId)),
    });
    ...
    return (this.db as any).query.purchaseOrders.findMany({
      where: and(eq((schema as any).purchaseOrders.vendorId, vendorId), eq((schema as any).purchaseOrders.tenantId, tenantId)),
      with: {
        items: true,
        vendor: true, // Assuming a vendor relation
      },
  ...
  async deletePurchaseOrder(id: string, tenantId: string): Promise<void> {
    return (this.db as any).transaction(async (tx) => {
      ...
      await tx.delete((schema as any).purchaseOrderItems)
        .where(eq((schema as any).purchaseOrderItems.purchaseOrderId, id));

      // Then delete the purchase order
      const [deletedOrder] = await tx.delete((schema as any).purchaseOrders)
        .where(and(eq((schema as any).purchaseOrders.id, id), eq((schema as any).purchaseOrders.tenantId, tenantId)))
        .returning({ id: (schema as any).purchaseOrders.id });
