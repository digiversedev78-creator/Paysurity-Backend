...
//   inventoryItems/purchaseOrders/purchaseOrderItems â€” not exported from purchase-orders.service
//   PG_CONNECTION from ../../drizzle/drizzle.provider â€” path doesn't exist
//   AuditLogService from ../../audit/audit-log.service â€” wrong path (real: ../audit-log/audit-log.service)
//   CreatePurchaseOrderDto etc from ./dto/purchase-order.dto â€” path doesn't exist
...
describe('PurchaseOrdersService', () => {
  // typed as jest.Mocked<any> â€” tests call phantom methods (getPurchaseOrderById, getAllPurchaseOrders,
  ...
  describe('createPurchaseOrder', () => {
    ...
    it('should create a purchase order successfully', async () => {
      ...
      jest.spyOn(service, 'getPurchaseOrderById').mockResolvedValue({
        ...
        items: (createDto as any).items.map(item => ({...item, unitPrice: parseFloat(String(item.unitPrice))}))
      } as any);
  ...
  describe('updatePurchaseOrder', () => {
    ...
    it('should update a purchase order successfully', async () => {
      ...
      db.update.mockImplementationOnce(() => ({ // For purchaseOrders update
        ...
        returning: jest.fn().mockResolvedValue([{ ...existingPo, ...updateDto, updatedAt: new Date(), status: (updateDto as any).status }]),
      }));
      ...
      jest.spyOn(service, 'getPurchaseOrderById').mockResolvedValue({
        ...existingPo, ...updateDto, updatedAt: new Date(), status: (updateDto as any).status,
        items: (updateDto as any).items.map(item => ({...item, unitPrice: parseFloat(String(item.unitPrice))}))
      } as any);
