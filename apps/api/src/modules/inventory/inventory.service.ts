...
export class InventoryService {
  ...
  async updatePrice(tenantId: string, userId: string, itemId: string, newPrice: number, type: 'RETAIL' | 'RESTAURANT') {
    ...
    const existing = await (this.db as any).select().from(table).where(and(eq(table.id, itemId), eq(table.tenantId, tenantId))).limit(1);
    ...
    const [updated] = await (this.db as any).update(table)
      .set(updateData)
      .where(and(eq(table.id, itemId), eq(table.tenantId, tenantId)))
      .returning();

    // 3. Audit Log (with Impersonation awareness if needed)
    await (this.auditLogService as any).logActivity(tenantId, userId, 'INVENTORY', `UPDATE_PRICE_${type}`, {
      itemId,
  ...
  async toggleActive(tenantId: string, userId: string, itemId: string, isActive: boolean, type: 'RETAIL' | 'RESTAURANT') {
    ...
    await (this.auditLogService as any).logActivity(tenantId, userId, 'INVENTORY', `TOGGLE_ACTIVE_${type}`, {
      itemId,
      isActive,
      isImpersonating: (global as any).isImpersonating || false
    });
