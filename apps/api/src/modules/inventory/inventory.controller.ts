...
export class InventoryController {
  ...
  async findAll(@Request() req, @Query('barcode') barcode?: string) {
    ...
    let query = (this.db as any).select().from(products).where(eq(products.tenantId, tenantId)).$dynamic();
  ...
  async create(@Request() req, @Body() createProductDto: CreateProductDto) {
    ...
    try {
      const [newProduct] = await (this.db as any).insert(products).values({
        ...createProductDto,
        tenantId: tenantId,
        // Drizzle numeric type expects string representation for cost and price
        cost: String((createProductDto as any).cost),
        price: String((createProductDto as any).price),
      }).returning().execute();
      ...
      if (error instanceof Error && error.message.includes('tenant_barcode_idx')) {
        throw new ConflictException(`Product with barcode "${(createProductDto as any).barcode}" already exists for this tenant.`);
      }
  ...
  async update(@Request() req, @Param('id', new ParseUUIDPipe()) id: string, @Body() updateProductDto: UpdateProductDto) {
    ...
    if ((updateProductDto as any).cost !== undefined) {
        updateData.cost = String((updateProductDto as any).cost);
    }
    if ((updateProductDto as any).price !== undefined) {
        updateData.price = String((updateProductDto as any).price);
    }

    try {
      const [updatedProduct] = await (this.db as any).update(products)
        .set(updateData)
      ...
      if (error instanceof Error && error.message.includes('tenant_barcode_idx')) {
        throw new ConflictException(`Product with barcode "${(updateProductDto as any).barcode}" already exists for this tenant.`);
      }
      throw error;
    }
  }
  ...
  async remove(@Request() req, @Param('id', new ParseUUIDPipe()) id: string) {
    ...
    const [deletedProduct] = await (this.db as any).delete(products)
      .where(and(eq(products.id, id), eq(products.tenantId, tenantId)))
  ...
  async adjustStock(@Request() req, @Param('id', new ParseUUIDPipe()) id: string, @Body() stockAdjustDto: StockAdjustDto) {
    ...
    const productToUpdate = await (this.db as any).select()
      .from(products)
      .where(and(eq(products.id, id), eq(products.tenantId, tenantId)))
      .limit(1)
      .execute();
    ...
    if ((stockAdjustDto as any).operation === StockOperation.ADD) {
      newStock = currentStock + (stockAdjustDto as any).quantity;
    } else { // StockOperation.SUBTRACT
      if (currentStock < (stockAdjustDto as any).quantity) {
        throw new BadRequestException(`Cannot subtract ${(stockAdjustDto as any).quantity} from current stock level of ${currentStock}. Resulting stock would be negative.`);
      }
      newStock = currentStock - (stockAdjustDto as any).quantity;
    }
  ...
  async getLowStock(@Request() req) {
    ...
    const lowStockItems = await (this.db as any).select()
      .from(products)
