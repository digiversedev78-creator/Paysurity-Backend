...
class CheckoutService {
  ...
  async ringUpTransaction(params: RingUpTransactionParams): Promise<TransactionReceipt> {
    ...
    await (this.authService as any).verifyTenantId(tenantId, { storeId, cashierId });
    await (this.authService as any).hasPermission(cashierId, 'POS_TRANSACTION_RING_UP', { storeId, tenantId });
    ...
    try {
      ...
      for (const item of items) {
        ...
        await (this.productService as any).getProductDetails(tenantId, item.productId);
        totalAmount += item.quantity * item.unitPrice;
      }

      if (loyaltyId && totalAmount > 0) {
        try {
          const discount = await (this.loyaltyService as any).applyDiscount(tenantId, loyaltyId, items, discountCode);
      ...
      for (const item of items) {
        await (this.inventoryService as any).reserveStock(tenantId, item.productId, item.quantity);
      }
      ...
      for (const item of items) {
        await (this.inventoryService as any).deductStock(tenantId, item.productId, item.quantity);
      }

      if (loyaltyId) {
        try {
          const pointsEarned = Math.floor(totalAmount / 10); // Example logic
          await (this.loyaltyService as any).addLoyaltyPoints(tenantId, loyaltyId, pointsEarned);
        } catch (error: any) {
      ...
      if (items && items.length > 0) {
        for (const item of items) {
          await (this.inventoryService as any).releaseStock(tenantId, item.productId, item.quantity).catch(console.error);
        }
      }
      throw error;
    }
