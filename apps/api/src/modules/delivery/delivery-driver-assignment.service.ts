...
import { orders, } /* (schema as any).deliveryDrivers removed */ from '@paysurity/database';
...
export class DeliveryDriverAssignmentService {
  ...
  async registerDriver(driverData: Omit<InsertDriver, 'id' | 'createdAt' | 'updatedAt' | 'tenantId'>, tenantId: string): Promise<Driver> {
    ...
    return ((result as any).rows as any)[0] as Driver;
  }
  ...
  async updateDriverProfile(driverId: string, updateData: UpdateDriver, tenantId: string): Promise<Driver | null> {
    ...
    return (result as any).rows.length > 0 ? ((result as any).rows[0] as Driver) : null;
  }
  ...
  async getDrivers(tenantId: string, status?: DriverStatus): Promise<Driver[]> {
    ...
    const result = await (this.db as any).execute(selectQuery, params);
    return (result as any).rows as Driver[];
  }
  ...
  async assignDriverToOrder(orderId: string, driverId: string, tenantId: string): Promise<Order | null> {
    ...
    if ((driverResult as any).rows.length === 0) {
    ...
    try {
      ...
      if ((updatedOrdersResult as any).rows.length === 0) {
        // Check why it failed
        ...
        if ((existingOrderResult as any).rows.length === 0) {
          throw new NotFoundException('Order not found.');
        } else if ((existingOrderResult as any).rows[0].driver_id) {
      ...
      await client.commit();
      return ((updatedOrdersResult as any).rows as any)[0] as Order;
  ...
  async getAssignmentByOrderId(orderId: string, tenantId: string): Promise<(Order & { driver?: Driver }) | null> {
    ...
    const row: QueryResultRow = (result as any).rows[0];
    const order: Order = {
      id: row.id,
    ...
    return { ...order, driver: driver || undefined };
  }
  ...
  async getAssignmentsByDriverId(driverId: string, tenantId: string): Promise<Order[]> {
    ...
    const result = await (this.db as any).execute(selectQuery, params);
    return (result as any).rows as Order[];
  }
  ...
  async updateDriverAssignment(orderId: string, newDriverId: string | null, tenantId: string): Promise<Order | null> {
    ...
    try {
      ...
      if ((currentOrderResult as any).rows.length === 0) {
        throw new NotFoundException('Order not found.');
      }
      currentDriverId = (currentOrderResult as any).rows[0].driver_id;
      ...
      if (newDriverId !== null) {
        ...
        if ((newDriverResult as any).rows.length === 0) {
          throw new NotFoundException('New delivery driver not found.');
        }
        const newDriver = (newDriverResult as any).rows[0];
      ...
      if (currentDriverId && currentDriverId !== newDriverId) {
        ...
        const oldDriverNewStatus = ((activeLoadsForOldDriver as any).rows[0].count > 0) ? 'on-load' : 'available';
