...
export class RestaurantAnalyticsService {
  ...
  async getDailyRevenueSummary(dto: AnalyticsQueryDto): Promise<any> {
    await (this.auditLogService as any).record(dto.tenantId, {
      userId: dto.userId,
    ...
    const currentPeriodResult = await (this.db as any).execute(queryBuilder(dto.startDate, dto.endDate));
    let comparisonPeriodResult;

    if (dto.compareStartDate && dto.compareEndDate) {
      comparisonPeriodResult = await (this.db as any).execute(queryBuilder(dto.compareStartDate, dto.compareEndDate));
    }

    return {
      current: (currentPeriodResult as any).rows,
  ...
  async calculateLoyaltyPoints(orderId: string, tenantId: string, userId?: string): Promise<{ pointsEarned: number; loyaltyRate: number }> {
    ...
    `;
    const orderTotalResult = await (this.db as any).execute(orderTotalQuery);
    const order = (orderTotalResult as any).rows[0];
