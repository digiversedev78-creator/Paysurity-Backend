...
export class WebhookService {
  ...
  async findAll(tenantId: string): Promise<any[]> {
    if (this.db && webhookTable) {
      try {
        return await (this.db as any).select().from(webhookTable)
  ...
  async findById(id: string, tenantId: string): Promise<any> {
    if (this.db && webhookTable) {
      try {
        const rows = await (this.db as any).select().from(webhookTable)
  ...
  async create(tenantId: string, data: Record<string, any>): Promise<any> {
    ...
    if (this.db && webhookTable) {
      try {
        const [row] = await (this.db as any).insert(webhookTable).values({
          id, tenantId, ...data,
        }).returning();
        return row;
      } catch (err) {
  ...
  async update(id: string, tenantId: string, data: Record<string, any>): Promise<any> {
    if (this.db && webhookTable) {
      try {
        const [row] = await (this.db as any).update(webhookTable)
  ...
  async delete(id: string, tenantId: string): Promise<boolean> {
    if (this.db && webhookTable) {
      try {
        await (this.db as any).delete(webhookTable)
          .where(and(eq(webhookTable.id, id), eq(webhookTable.tenantId, tenantId)));
        return true;
      } catch (err) {
