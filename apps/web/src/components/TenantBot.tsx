 ...
 * Priority order:
 *  1. Specialty keyword match (most specific — e.g. "biryani" → biryani reply)
 *  2. FAQ keyword match (tenant-specific Q&A)
 *  3. Halal / dietary
 *  4. Catering / events
 *  5. Hours
 *  6. Location / address
 *  7. Phone / contact
 *  8. Generic menu highlights
 *  9. Default fallback
 */
function buildReply(input: string, k: BotKnowledge): string {
  const lower = input.toLowerCase();

  // 1. Specialty keywords — check FIRST so "tell me about biryani" gets a specific response
  if (k.specialties) {
  ...
  // 2. FAQ matching — fuzzy word overlap
  if (k.faqs) {
...
export default function TenantBot({ botName, accent, knowledge }: TenantBotProps) {
  ...
  const send = useCallback(async () => {
    ...
    const reply = buildReply(text, knowledge);
    setMessages(p => [...p, { role: 'bot', text: reply }]);
    setLoading(false);
  }, [input, loading, knowledge]);
