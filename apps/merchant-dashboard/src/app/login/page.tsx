...
export default function LoginPage() {
  ...
  // ── Core login logic (shared by demo buttons and manual form) ──
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (tok) { setToken(tok); router.push('/dashboard'); }
    } catch (err: any) {
  ...
  const handleMfa = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.accessToken) { setToken(data.accessToken); router.push('/dashboard'); }
    } catch (err: any) {
