...
export class AuthController {
  ...
  async login(@Body() loginDto: LoginDto, @Req() req: Request) {
    ...
    const tenantId = req.user?.tenantId ?? loginDto.tenantId;
    this.logger.debug(`Login attempt: ${loginDto.email}`);

    try {
      const result = await this.authService.login({ ...loginDto, tenantId }) as { accessToken?: string, userId?: string };
      ...
      this.logger.verbose(`Login for ${loginDto.email} in ${durationMs.toFixed(2)}ms`);
      return { accessToken };
    } catch (error) {
      this.logger.warn(`Login failed: ${loginDto.email} — ${error.message}`);
      throw error;
    }
  }
  ...
  async register(@Body() registerDto: RegisterDto, @Req() req: Request) {
    const tenantId = req.user?.tenantId ?? registerDto.tenantId;
    const startTime = process.hrtime.bigint();

    try {
      const result = await this.authService.register({ ...registerDto, tenantId, name: `${registerDto.firstName || ''} ${registerDto.lastName || ''}`.trim() || 'Unknown' }) as { id?: string, userId?: string };
      ...
      const durationMs = Number(process.hrtime.bigint() - startTime) / 1_000_000;
      this.logger.verbose(`Register for ${registerDto.email} in ${durationMs.toFixed(2)}ms`);
      return { userId };
    } catch (error) {
      this.logger.error(`Registration failed: ${registerDto.email} — ${error.message}`, error.stack);
      throw error;
    }
  }
  ...
  async refreshToken(@Body() refreshDto: RefreshTokenDto) {
    try {
      const result = await this.authService.refreshToken(refreshDto.refreshToken) as Record<string, unknown>;
      const accessToken = result?.accessToken;
      return { accessToken };
    } catch (error) {
