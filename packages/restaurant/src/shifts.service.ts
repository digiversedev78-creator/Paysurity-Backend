...
export class ShiftsService {
    ...
    async openShift(tenantId: string, employeeId: string): Promise<typeof shifts.$inferSelect> {
        ...
        // CRITICAL RULE #8: Use raw sql`` template literals
        const employeeResult = await this.db.execute(sql`
        ...
        // CRITICAL RULE #8: Use raw sql`` template literals
        const openShiftResult = await this.db.execute(sql`
        ...
        // CRITICAL RULE #8: Use raw sql`` template literals
        const insertedShift = await this.db.execute(sql`
        ...
        // CRITICAL RULE #9: AuditLogService
        this.auditLogService.record(tenantId, {
            userId: String(employeeId ?? ""),
            action: 'SHIFT_CLOCK_IN',
            details: { shiftId: (insertedShift.rows[0] as any).id, clockIn: clockInTime.toISOString() },
        });

        return insertedShift.rows[0] as any;
    }

    async closeShift(tenantId: string, shiftId: string): Promise<typeof shifts.$inferSelect> {
        // Fetch the shift
        // CRITICAL RULE #8: Use raw sql`` template literals
        const shiftResult = await this.db.execute(sql`
        ...
        // CRITICAL RULE #8: Use raw sql`` template literals
        const updatedShift = await this.db.execute(sql`
            UPDATE shifts
        ...
        return updatedShift.rows[0] as any;
    }
