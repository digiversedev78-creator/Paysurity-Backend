/**
 * RestaurantService — Production Grade v2.0
 *
 * REQ-POSR-001: Core Order Management — openOrder, addItem, fireToKDS, splitCheck, closeOrder, voidOrder
 * REQ-POSR-002: KDS routing — routeItemToStation() called from addItem + fireToKDS
 * REQ-POSR-003: Menu & modifier management — delegated to MenuService
 * REQ-POSR-005: Multi-channel aggregation — source_channel stored on every order
 * REQ-POSR-006: Payment processing & tip — multi-tender, auto-gratuity from DB config
 * REQ-POSR-007: Shift close report — closeShift() generates shift_close_reports row
 * REQ-POSR-008: Loyalty at POS — lookupLoyaltyAccount(), earning triggered inside closeOrder tx
 * REQ-POSR-009: Offline sync — processBatchSync() accepts array from queue
 * REQ-POSR-010: Receipt — generateReceiptPayload() for print/email/SMS
 *
...
export class RestaurantService {
  ...
  async addItem(params: AddItemParams): Promise<{ orderItemId: string; orderSubtotalCents: number }> {
    ...
    const orderResult = await this.db.execute(sql`
    ...
    const updatedOrder = await this.db.execute(sql`SELECT subtotal_cents FROM orders WHERE id = ${orderId}::uuid`);
  ...
  async fireToKDS(tenantId: string, orderId: string, courseNumber?: number): Promise<{ stationsFired: number; itemsFired: number }> {
    ...
    const itemsResult = await this.db.execute(itemsQuery);
  ...
  async splitCheck(tenantId: string, orderId: string, spec: SplitSpec): Promise<{ childOrderIds: string[] }> {
    ...
    if (spec.ways) {
      ...
      for (const split of spec.splits) {
        const childId = randomUUID();
        const itemResult = await this.db.execute(sql`
          SELECT SUM(total_cents) AS subtotal FROM order_items
  ...
  async voidOrder(tenantId: string, orderId: string, reason: string, managerId?: string): Promise<void> {
    ...
    this.logger.log(`REQ-POSR-001: Order ${order.short_id} voided — ${reason}`);
  }
  ...
  async processBatchSync(tenantId: string, locationId: string, offlineOrders: any[]): Promise<{
    ...
    this.logger.log(`REQ-POSR-009: Offline sync complete — ${synced} synced, ${failed} failed`);
