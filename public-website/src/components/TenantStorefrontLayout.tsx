...
export default function TenantStorefrontLayout({ config, children, hideHero = false }: { config: StorefrontProps; children?: React.ReactNode; hideHero?: boolean }) {
  ...
  useEffect(() => {
    // Fetch dynamic product catalog for AI bot and Hero Carousel
    const slugMatch = config.navLinks[0]?.href.split('/').pop();
    const slug = slugMatch || config.name.toLowerCase().replace(/[^a-z0-9]+/g, '-');
    const apiUrl = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001';

    fetch(`${apiUrl}/api/website/${slug}/products`)
      .then(res => res.json())
      .then(data => {
        if (data && data.data) {
  ...
  }, [config.name, config.navLinks]);

  // Use dynamically fetched products if available, fallback to config
  const dynamicSlides: CarouselSlide[] = products.length > 0 
    ? products.slice(0, 5).map((p, index) => ({
        icon: '🛍️',
        title: p.name,
        subtitle: p.category_name || 'Featured',
        desc: p.description || '',
        href: `${config.navLinks[0]?.href}/catalog?productId=${p.id}`,
        imageUrl: config.carousel[index % config.carousel.length]?.imageUrl 
      }))
    : config.carousel;
      ...
      <header className="relative min-h-screen flex flex-col items-center justify-center text-center px-4 pt-32 md:pt-48 pb-20 overflow-hidden">
        ...
        <div className="relative z-10 max-w-5xl mx-auto w-full">
          ...
          <div className="w-full">
            ...
            <TenantCarousel slides={dynamicSlides} accent={config.accentColor} />
          </div>
        </div>
      </header>
      )}
