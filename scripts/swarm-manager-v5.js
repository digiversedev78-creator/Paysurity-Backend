 ...
 * ─────────────────────────────────────────────────────────────────
 * PaySurity Platform Swarm V5 — 228-Requirement Manager
 *
 ...
 *  - process.env.MANIFEST_FILE || 'REQUIREMENTS_V5_MANIFEST.json' updated per-requirement
...
const MANIFEST_PATH = path.join(ROOT, process.env.MANIFEST_FILE || 'REQUIREMENTS_V5_MANIFEST.json');
...
Respond with ONLY valid JSON (no markdown fences):
{
  ...
  try {
    ...
    // Strategy 2: sanitize control characters (the main culprit)
    // Replace raw control chars (tab, newline etc) inside the JSON string values
    const sanitized = text.replace(/[\x00-\x08\x0b\x0c\x0e-\x1f]/g, '');
    try { return JSON.parse(sanitized); } catch (_) {}

    // Strategy 3: Fix bad escape sequences (Gemini sometimes outputs \d, \s etc)
    const fixed = sanitized.replace(/\\([^"\\\/bfnrtu])/g, '\\\\$1');
    try { return JSON.parse(fixed); } catch (_) {}

    // Strategy 4: Extract each field individually via regex (last resort)
    const extract = (field) => {
    ...
    const partial = {
      ...
      controller:  extract('controller'),
      module_file: extract('module_file'),
      dto:         extract('dto'),
      test:        extract('test'),
    };
    if (partial.service || partial.controller || partial.test) return partial;

    throw new Error(`All JSON parse strategies failed for ${req.id}`);
  } catch (e) {
